...
__qpu__ void park_QFT(int num_qubits,
  ...
  variational_ansatz(q, depth, params,
                     ...
                     /*reverse=*/true);
  QFT_LNN(q, 1, num_qubits1Dpark);
  h(q[0]);

  for (int idx = 0; idx < num_qubits1Dpark; ++idx) {
    double theta = (2 * M_PI / (1 << num_qubits1Dpark)) * (1 << idx);
    cp(theta, q[0], q[num_qubits1Dpark - idx]);
  }
  h(q[0]);
  mz(q[0]);
}
...
double objective_cb(unsigned n, const double* x, double* /*grad*/, void* ptr){
  ...
  auto den_res = cudaq::sample(d->shots, park_QFT,
                               d->nq, d->depth, theta);
  
  double p0 = den_res.count("0") / static_cast<double>(d->shots);
  double p1 = den_res.count("1") / static_cast<double>(d->shots);
  double denom = 2.0 * (p0 - p1) - 2.0;

  double diridenom = 0.0;
  for (int j = 0; j < (d->num_qubits1D); j++) {
    auto den_res2 = cudaq::sample(d->shots, laplacian_dirichlet, d->nq, d->num_qubits1D, d->depth, j, theta);
    if (j == (d->num_qubits1D) - 1) {
      double p0 = den_res2.count("0") / static_cast<double>(d->shots);
      double p1 = den_res2.count("1") / static_cast<double>(d->shots);
      diridenom = diridenom + (p0 - p1);
    } else {
      const std::string tail((d->num_qubits1D) - j - 2, '0');
      const std::string key0 = tail + "1" "0";  // == "01" + tail
      const std::string key1 = tail + "1" "1";  // == "11" + tail
      double p0 = den_res2.count(key0) / static_cast<double>(d->shots);
      double p1 = den_res2.count(key1) / static_cast<double>(d->shots);
      diridenom = diridenom +  (p0 - p1);
    }
  }
  diridenom = diridenom - 2.0;
  denom = denom + diridenom;
  ...
  if (d->eval_cnt % (d->dim + 1) == 0) {
    std::string evs_name = "../log/evs_log_FD_LNN_" + d->prefix + ".txt";
    std::string trace_name = "../log/trace_log_FD_LNN_" + d->prefix + ".txt";
    std::string l2_name = "../log/l2_log_FD_LNN_" + d->prefix + ".txt";
...
int main(int argc, char* argv[]){
  ...
  std::ofstream("../log/evs_log_FD_LNN_" + data.prefix + ".txt", std::ios::trunc).close();
  std::ofstream("../log/trace_log_FD_LNN_" + data.prefix + ".txt", std::ios::trunc).close();
  std::ofstream("../log/l2_log_FD_LNN_" + data.prefix + ".txt", std::ios::trunc).close();
...
{
    std::ofstream evs("../log/evs_log_FD_LNN_" + data.prefix + ".txt", std::ios::app);
    std::ofstream tr("../log/trace_log_FD_LNN_" + data.prefix + ".txt", std::ios::app);
    std::ofstream l2("../log/l2_log_FD_LNN_" + data.prefix + ".txt", std::ios::app);
